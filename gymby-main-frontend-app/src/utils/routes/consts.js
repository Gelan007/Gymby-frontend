export const HOME_ROUTE = "/";
export const LOGIN_ROUTE = "/login";

export const USER_ACCOUNT_ROUTE = "/userAccount";
export const USER_ACCOUNT_PERSONAL_DATA_ROUTE = USER_ACCOUNT_ROUTE + "/personalData";
export const USER_ACCOUNT_COACHING_ROUTE = USER_ACCOUNT_ROUTE + "/coaching";
export const USER_ACCOUNT_FRIENDS_LIST_ROUTE = USER_ACCOUNT_ROUTE + "/friendsList";
export const USER_ACCOUNT_INSTRUCTION_ROUTE = USER_ACCOUNT_ROUTE + "/instruction";
export const USER_ACCOUNT_SUBSCRIBES_ROUTE = USER_ACCOUNT_ROUTE + "/subscribes";
export const USER_ACCOUNT_SEARCH_ROUTE = USER_ACCOUNT_ROUTE + "/search";
export const USER_ACCOUNT_PROFILE_ROUTE = USER_ACCOUNT_ROUTE + "/profile/:username";


export const MEASUREMENTS_ROUTE = "/measurements";
export const MEASUREMENTS_HAND_ROUTE = MEASUREMENTS_ROUTE + "/hand";
export const MEASUREMENTS_WEIGHT_ROUTE = MEASUREMENTS_ROUTE + "/weight";
export const MEASUREMENTS_FAT_PERCENTAGE_ROUTE = MEASUREMENTS_ROUTE + "/fatPercentage";
export const MEASUREMENTS_NECK_ROUTE = MEASUREMENTS_ROUTE + "/neck";
export const MEASUREMENTS_SHOULDERS_ROUTE = MEASUREMENTS_ROUTE + "/shoulders";
export const MEASUREMENTS_FOREARMS_ROUTE = MEASUREMENTS_ROUTE + "/forearms";
export const MEASUREMENTS_CHEST_ROUTE = MEASUREMENTS_ROUTE + "/chest";
export const MEASUREMENTS_WAIST_ROUTE = MEASUREMENTS_ROUTE + "/waist";
export const MEASUREMENTS_THIGH_ROUTE = MEASUREMENTS_ROUTE + "/thigh";
export const MEASUREMENTS_PHOTOS_ROUTE = MEASUREMENTS_ROUTE + "/photos";

export const DIARY_ROUTE = "/diary";

export const PROGRAMS_ROUTE = "/programs";
export const PROGRAMS_FREE_ROUTE = PROGRAMS_ROUTE + "/free";
export const PROGRAMS_FAVORITES_ROUTE = PROGRAMS_ROUTE + "/favorites";
export const PROGRAMS_PERSONAL_ROUTE = PROGRAMS_ROUTE + "/personal";
export const PROGRAMS_TRAINER_BASED_ROUTE = PROGRAMS_ROUTE + "/trainerBased";
export const PROGRAMS_PROGRAM_PROFILE_ROUTE = PROGRAMS_ROUTE + "/:programId";

export const STATISTICS_ROUTE = "/statistics";


